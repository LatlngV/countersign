...
class LeaderCountersign(models.Model):
    ...
    title = fields.Char(string=u"标题", default="付款申请表", readonly=True)
    project_name = fields.Char(string=u"工程名称", required=True)
    subcontract_item = fields.Char(string=u"分包项目", required=True)
    ...
    apply_detail_line = fields.One2many("apply.detail", "countersign_id", string=u"申请详情", required=True,
                                        ondelete="set null")
    leader_line = fields.One2many("leader.line", "leader_line_id", string=u"会签领导", required=True, ondelete="set null")
    ...
    payer = fields.Char(string=u"付款人", compute="_compute_payer_info", store=True)
    payment_account = fields.Char(string=u"付款账户")
    payment_date = fields.Date(string=u"付款日期", compute="_compute_payer_info", store=True)
    payment_money = fields.Float(string=u"付款金额", digits=(10, 1))
    payment = fields.Boolean(string=u"是否付款", default=False)

    @api.depends("payment_account")
    def _compute_payer_info(self):
        for record in self:
            if record.payment_account:
                ...
                for employee in employee_id:
                    if user_id == employee.user_id.id:
                        record.payer = employee.user_id.name
        ...
        """
            计算状态:
                ...
                2. 如果是就判断当前日期是不是超过设定日期
                    1.) 设定日期: 创建申请单的日期加上一天
                    2.) 如果是就将状态设置为 “结束”
                    3.) 如果 确认+拒绝 的数量等于数据库里存的 count，直接 return
                    4.) 然后 for 遍历状态，如果有状态就跳过，没有就把未选择的状态写到数据库
        """
        for record in self:
            ...
            if current_date > end_date:
                ...
                if record.count == len(record.leader_line):
                    continue
                for line in record.leader_line:
                    if line.state == "confirm" or line.state == "cancel":
                        continue
                    else:
                        line.write({"state": "no_select"})
    ...
    def action_print(self):
        """
            打印报表
        """
        if self.final_state == "confirm" or self.final_state == "no_confirm":
            return self.env["report"].get_action(self, "countersign.report_countersign_view")
...
class ApplyDetail(models.Model):
    ...
    detail = fields.Char(string=u"摘要", required=True)
...
class LeaderLine(models.Model):
    ...
    SELECT_STATE = [("confirm", u"确认"), ("cancel", u"拒绝"), ("no_select", u"未选择")]
    ...
    datetime = fields.Datetime(string=u"日期", readonly=True)
